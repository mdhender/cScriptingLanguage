...
void ZAsyncDeviceInt::getLineCtl(LINECONTROL& param)
{
   ...
   APIRET rc =
      DosDevIOCtl(
         iHandle, IOCTL_ASYNC, ASYNC_GETLINECTRL,
         NULL, 0, NULL,
...
void ZAsyncDeviceInt::setLineCtl(LINECONTROL& param)
{
   ...
   APIRET rc =
      DosDevIOCtl(
         iHandle, IOCTL_ASYNC, ASYNC_SETLINECTRL,
         &param, sizeof(param), &parLen,
         NULL, 0, NULL);
   if (rc != NO_ERROR)
...
void ZAsyncDeviceInt::getDcbInfo(DCBINFO& param)
{
   ...
   APIRET rc =
      DosDevIOCtl(
         iHandle, IOCTL_ASYNC, ASYNC_GETDCBINFO,
         NULL, 0, NULL,
         &param, parLen, &parLen);
   if (rc != NO_ERROR)
...
void ZAsyncDeviceInt::setDcbInfo(DCBINFO& param)
{
   ...
   APIRET rc =
      DosDevIOCtl(
         iHandle, IOCTL_ASYNC, ASYNC_SETDCBINFO,
         &param, parLen, &parLen,
         NULL, 0, NULL);
   if (rc != NO_ERROR)
...
void ZAsyncDeviceInt::checkHandle()
{
   ...
   APIRET rc =
      DosOpen(
         (char*)iDevName, &iHandle, &action, 0L,
   ...
   UCHAR FlwClrBits = MODE_AUTO_TRANSMIT   |
                      MODE_AUTO_RECEIVE    |
                      MODE_ERROR_CHAR      |
                      MODE_NULL_STRIPPING  |
                      MODE_BREAK_CHAR      |
                      0x20                 | // mode_auto_receive_fullduplex
                      MODE_RTS_HANDSHAKE;
   ...
   rc = DosDevIOCtl(
            iHandle, IOCTL_ASYNC, ASYNC_SETMODEMCTRL,
   ...
   // start transmitting
   rc = DosDevIOCtl(
            iHandle, IOCTL_ASYNC, ASYNC_STARTTRANSMIT,
   ...
   rc = DosDevIOCtl(
            iHandle, IOCTL_ASYNC, ASYNC_SETBREAKOFF,
            NULL, 0, NULL,
            &errWord, errLen, &errLen);
   if (rc != NO_ERROR)
...
void ZAsyncDeviceInt::waitForOutput()
{
   ...
   for (;;) {
      ...
      APIRET rc;
      rc = DosDevIOCtl(
               iHandle, IOCTL_ASYNC, ASYNC_GETLINESTATUS,
               NULL, 0, NULL,
               &status, parLen, &parLen);
      if (rc != NO_ERROR)
...
long ZAsyncDeviceInt::inputChars()
{
   ...
   APIRET rc =
      DosDevIOCtl(
         iHandle, IOCTL_ASYNC, ASYNC_GETINQUECOUNT,
         NULL, 0, NULL,
         &rxq, rxqlen, &rxqlen);
   if (rc != NO_ERROR)
...
void ZAsyncDeviceInt::setBps(long aBps)
{
   ...
   param.fraction = 0;
   rc = DosDevIOCtl(
            iHandle, IOCTL_ASYNC, ASYNC_EXTSETBAUDRATE,
            &param, sizeof(param), &parLen,
            NULL, 0, NULL);
   if (rc != NO_ERROR)
...
{
   ZFUNCTRACE_DEVELOP("ZAsyncDeviceInt::setBits(ZAsyncDevice::DataBits bits)");
   checkHandle();
   LINECONTROL param;
   getLineCtl(param);
...
{
   ZFUNCTRACE_DEVELOP("ZAsyncDeviceInt::setStops(ZAsyncDevice::StopBits stp)");
   checkHandle();
   LINECONTROL param;
   getLineCtl(param);
   param.bStopBits = stp;
...
{
   ZFUNCTRACE_DEVELOP("ZAsyncDeviceInt::setFlow(ZAsyncDevice::FlowCtrl flow)");
   if (flow != iFlow) {
      closeHandle();
      iFlow = flow;
   } // if
...
long ZAsyncDeviceInt::bps()
{
   ...
   checkHandle();
   rc = DosDevIOCtl(
            iHandle, IOCTL_ASYNC, ASYNC_EXTGETBAUDRATE,
            NULL, 0, NULL,
            &param, sizeof(param), &parLen);
   if (rc != NO_ERROR)
      ZTHROWASYNCOS2(" setting bps");
...
ZAsyncDeviceInt::ZAsyncDeviceInt(ZAsyncDevice* aParent, const ZString& aDevName) :
   iReadTimeout(1000),
   iBps(9600),
   iDataBits(ZAsyncDevice::dataBits8),
   iParity(ZAsyncDevice::parityNone),
   iStopBits(ZAsyncDevice::stopBits1),
   iFlow(ZAsyncDevice::flowNone),
   iIsOpen(zFalse),
   iDevName(aDevName),
   iParent(aParent)
{
   ZFUNCTRACE_DEVELOP("ZAsyncDeviceInt::ZAsyncDeviceInt(ZAsyncDevice* aParent, const ZString& aDevName)");
} // ZAsyncDeviceInt

ZAsyncDeviceInt::~ZAsyncDeviceInt()
{
   ZFUNCTRACE_DEVELOP("ZAsyncDeviceInt::~ZAsyncDeviceInt()");
   if (iIsOpen) CloseHandle(iHandle);
} // ~ZAsyncDeviceInt
...
void ZAsyncDeviceInt::checkHandle()
{
   ...
   while (!iIsOpen) {
      try {
         ...
         dcb.BaudRate          = iBps;
         dcb.fBinary           = TRUE;
         dcb.fOutxCtsFlow      = (iFlow==ZAsyncDevice::flowHard || iFlow==ZAsyncDevice::flowBoth);
         dcb.fOutxDsrFlow      = FALSE;
         dcb.fDtrControl       = DTR_CONTROL_ENABLE;
         dcb.fDsrSensitivity   = FALSE;
         dcb.fTXContinueOnXoff = TRUE;
         dcb.fOutX             =
         dcb.fInX              = (iFlow==ZAsyncDevice::flowSoft || iFlow==ZAsyncDevice::flowBoth);
         ...
         if (iHandle != INVALID_HANDLE_VALUE) CloseHandle(iHandle);
         if (++errCount == 10) throw;
      } // catch
   } // while
} // checkHandle
...
ZBoolean ZAsyncDeviceInt::checkError()
{
   ...
   if (errors & CE_DNS     ) ZTHROWASYNC(iDevName+" device not selected");
   if (errors & CE_FRAME   ) ZTHROWASYNC(iDevName+" framing error");
   if (errors & CE_IOE     ) ZTHROWASYNC(iDevName+" I/O error detected");
   if (errors & CE_MODE    ) ZTHROWASYNC(iDevName+" invalid mode requested");
   if (errors & CE_OOP     ) ZTHROWASYNC(iDevName+" out of paper");
   if (errors & CE_OVERRUN ) ZTHROWASYNC(iDevName+" char buffer overrun");
   if (errors & CE_PTO     ) ZTHROWASYNC(iDevName+" parallel port timeout");
   if (errors & CE_RXOVER  ) ZTHROWASYNC(iDevName+" input buffer overflow");
   if (errors & CE_RXPARITY) ZTHROWASYNC(iDevName+" parity error");
   if (errors & CE_TXFULL  ) ZTHROWASYNC(iDevName+" output buffer full");
   if (status.fEof)          ZTHROWASYNC(iDevName+" EOF received");
   return zTrue;
} // checkError
...
void ZAsyncDeviceInt::write(const ZString& aData)
{
   ...
   checkHandle();
   DWORD actual;
   BOOL rc = WriteFile(iHandle, (const char*)aData, aData.size(), &actual, NULL);
   if (!rc) ZTHROWASYNCWIN("WriteFile");
   checkError();
} // write
...
ZString ZAsyncDeviceInt::read(long aMaxChars)
{
   ...
   long    wtime(0);
   do {
      if (checkError()) { // any input chars waiting?
...
void ZAsyncDeviceInt::checkHandle()
{
...
#endif
      default: ZTHROWASYNC("invalid baud rate");
   } // switch

   switch (iDataBits) {
      case ZAsyncDevice::dataBits5: tio.c_cflag |= CS5; break;
      case ZAsyncDevice::dataBits6: tio.c_cflag |= CS6; break;
      case ZAsyncDevice::dataBits7: tio.c_cflag |= CS7; break;
   ...
   switch (iParity) {
      case ZAsyncDevice::parityEven: tio.c_cflag |= PARENB; break;
      case ZAsyncDevice::parityOdd:  tio.c_cflag |= PARENB | PARODD; break;
...
ZString ZAsyncDeviceInt::read(long aMaxChars)
{
   ...
   if (iReadTimeout) {
      ...
      if (!FD_ISSET(iHandle, &readset)) return ZString();
   } // if
   char* readbuf = new char[aMaxChars];
...
ZExport0 ZAsyncDevice::ZAsyncDevice(const ZString& aDevName, const ZString& aLogName)
{
   ZFUNCTRACE_DEVELOP("ZAsyncDevice::ZAsyncDevice(const ZString& aDevName, const ZString& aLogName)");
   if (aLogName.length()) {
...
ZExport(ZString) ZAsyncDevice::read(long aMaxChars)
{
   ...
   if (cnt > 0) {
      if (cnt > aMaxChars) cnt = aMaxChars;
