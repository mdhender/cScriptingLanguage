...
class ZDllModule : public ZBase
{
   public:
      ...
      // queries
      ZString name() const { return iName; }
      ZString fullName();

      // reference counting
      void addRef() { iUseCount++; }
      void removeRef() { iUseCount--; }
      unsigned long useCount() const { return iUseCount; }
      ...
      // list pointers
      ZDllModule* next() const { return iNext; }
      void setNext(ZDllModule* aNext) { iNext = aNext; }
...
ZDllModule::ZDllModule(const char* aName, ZDllModule* aNext) :
   ...
   iNext(aNext)
{
   ZFUNCTRACE_DEVELOP("ZDllModule::ZDllModule(const char* aName, ZDllModule* aNext)");
} // ZDllModule
...
ZExport0 ZDynamicLinkLibrary::ZDynamicLinkLibrary(const char* aName) :
   iModule(0)
{
   ZFUNCTRACE_DEVELOP("ZDynamicLinkLibrary::ZDynamicLinkLibrary(const char* aName)");
...
ZExport0 ZDynamicLinkLibrary::ZDynamicLinkLibrary(const ZDynamicLinkLibrary& aDll) :
   iModule(aDll.iModule)
{
   ZFUNCTRACE_DEVELOP("ZDynamicLinkLibrary::ZDynamicLinkLibrary(const ZDynamicLinkLibrary& aDll)");
   iModule->addRef();
} // ZDynamicLinkLibrary
...
ZExport(ZDynamicLinkLibrary&) ZDynamicLinkLibrary::operator=(const ZDynamicLinkLibrary& aDll)
{
   ZFUNCTRACE_DEVELOP("ZDynamicLinkLibrary::operator=(const ZDynamicLinkLibrary& aDll)");
   if (this != &aDll) {
      dropModule();
      iModule = aDll.iModule;
      iModule->addRef();
...
{
   ZFUNCTRACE_DEVELOP("ZDynamicLinkLibrary::procAddress(const char* aProcName)");
   iModule->open();
#if ZC_OS2
   int (_System *procAddr)();
   unsigned long rc = DosQueryProcAddr(iModule->iHandle, 0, aProcName, &procAddr);
   if (rc)
...
{
   ZFUNCTRACE_DEVELOP("ZDynamicLinkLibrary :: procAddress(unsigned long aProcOrdinal)");
   iModule->open();
#if ZC_OS2
   int (_System *procAddr)();
   unsigned long rc = DosQueryProcAddr(iModule->iHandle, aProcOrdinal, 0, &procAddr);
   if (rc)
      ZTHROWEXC("DosQueryProcAddr error "+ZString(rc)+" on #"+ZString(aProcOrdinal));
#endif
#if ZC_WIN
...
#if ZC_UNIXFAM
   dlerror(); // clear error code
   void* procAddr = dlsym(iModule->iHandle, ZString("@"+ZString(aProcOrdinal)).constBuffer());
   const char* err = dlerror();
   if (err != 0)
      ZTHROWEXC(err);
#endif
...
ZExport(ZBoolean) ZDynamicLinkLibrary::isEntryPoint32Bit(const char *aProcName)
{
   ZFUNCTRACE_DEVELOP("ZDynamicLinkLibrary::isEntryPoint32Bit(const char *aProcName)");
#if ZC_OS2
   iModule->open();
   unsigned long result(1);
   unsigned long rc = DosQueryProcType(iModule->iHandle, 0, aProcName, &result);
   if (rc)
...
ZExport(ZBoolean) ZDynamicLinkLibrary::isEntryPoint32Bit(unsigned long aProcOrdinal)
{
   ZFUNCTRACE_DEVELOP("ZDynamicLinkLibrary::isEntryPoint32Bit(unsigned long aProcOrdinal)");
#if ZC_OS2
   iModule->open();
   unsigned long result(1);
   unsigned long rc = DosQueryProcType(iModule->iHandle, aProcOrdinal, 0, &result);
   if (rc)
      ZTHROWEXC("DosQueryProcType error "+ZString(rc)+" on #"+ZString(aProcOrdinal));
   return (ZBoolean)result;
#else
   return zTrue;
#endif
