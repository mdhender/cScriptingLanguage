...
ZString ZProfile::getOs2Error() const
{
   ...
   switch (ERRORIDSEV(err)) {
      case 0x0000: msg = "No Error - "; break;
      case 0x0004: msg = "Warning - "; break;
      case 0x000C: msg = "Severe - "; break;
      case 0x0010: msg = "Unrecoverable - "; break;
      case 0x0008:
   ...
   switch (ERRORIDERROR(err)) {
      ...
      case 0x1303:
         ...
         break;
      case 0x1304:
         msg += "The application name, key name or program handle was not found.";
         break;
      case 0x110B:
...
typedef struct { const char* iName; HKEY iHandle; } SystemHandle;
...
static ZBoolean isSystemHandle(HKEY aHandle)
{
   ...
   for (unsigned i = 0; i < sizeof(sysHandles)/sizeof(SystemHandle); i++)
      if (aHandle == sysHandles[i].iHandle) return zTrue;
   return zFalse;
} // isSystemHandle
...
static void closeHandle(HKEY aHandle)
{
   ...
   if (!isSystemHandle(aHandle)) RegCloseKey(aHandle);
} // closeHandle

static HKEY openHandle(HKEY aParentHandle, ZString aPath, ZBoolean aCreate)
{
   ZFUNCTRACE_DEVELOP("openHandle(HKEY aParentHandle, ZString aPath, ZBoolean aCreate)");
   ...
   for (;;) {
      ...
      if (aCreate) {
         if (RegCreateKeyEx(
                parentHandle, token.constBuffer(), 0, "",
                REG_OPTION_NON_VOLATILE, (KEY_READ | KEY_WRITE),
                NULL, &handle, NULL) != ERROR_SUCCESS ) {
            if (parentHandle != aParentHandle) closeHandle(parentHandle);
...
void ZProfile::openPath(ZBoolean aCreate)
{
   ...
   if (iPathHandle == 0 || (aCreate && iPathReadOnly)) {
      ...
      openRoot(aCreate);
      if (iPath.size())
         iPathHandle = openHandle(iRootHandle, iPath, aCreate);
      else
...
void ZProfile::closePath()
{
   ...
   if (iPathHandle) {
      if (iPathHandle != iRootHandle) closeHandle(iPathHandle);
      iPathHandle = 0;
   } // if
} // closePath
...
void ZProfile::openRoot(ZBoolean aCreate)
{
   ...
   if (iRootHandle == 0 || (aCreate && iRootReadOnly)) {
      ...
      while (!parentKey) {
         ...
         if (!parentKey)
            iRoot = ZString(sysHandles[0].iName)+(char)ZC_PATHSEPARATOR+iRoot;
      } // while
      if (path.length())
...
void ZProfile::closeRoot()
{
   ...
   if (iRootHandle) {
      closePath();
      closeHandle(iRootHandle);
      iRootHandle = 0;
   } // if
#endif
#ifdef ZC_OS2
   if (iRootHandle != 0 &&
       iRootHandle != HINI_SYSTEMPROFILE &&
...
{
   ZFUNCTRACE_DEVELOP("ZProfile::ZProfile(const ZString& aRoot, const ZString& aPath)");
   iRootHandle = 0;
#ifdef ZC_WIN
   iPathHandle = 0;
   ...
   if (iHab == 0) throwSysErr(WinInitializeName);
   iRoot = aRoot;
   iPath = aPath;
#endif
} // ZProfile
...
#ifdef ZC_WIN
   return ZProfile(ZString(sysHandles[0].iName)+(char)ZC_PATHSEPARATOR+Software, aPath);
#endif
#ifdef ZC_OS2
...
#ifdef ZC_WIN
   return ZProfile(ZString(sysHandles[1].iName)+(char)ZC_PATHSEPARATOR+Software, aPath);
#endif
#ifdef ZC_OS2
...
ZExport(ZProfile&) ZProfile::getKeys(ZStringlist& aList)
{
   ...
   for (unsigned long key = 0; ; key++) {
      ...
      if (RegEnumKeyEx(
             iPathHandle, key, buffer, (LPDWORD)&bufsize,
...
#endif
#ifdef ZC_OS2
   if (iPath.size()) return *this; // os/2 has no keys at this level
   ...
   if (size) {
      ...
      while (size) {
         ZString key;
         while (*s && size) { key += *s++; size--; }
         if (key.size()) aList.addAsLast(key);
         if (size) { s++; size--; }
      } // while
   } // if
#endif
...
#endif
#ifdef ZC_OS2
   if (!iPath.size()) return *this; // os/2 has no values at this level
...
ZExport(int) ZProfile::valueType(const ZString& aValueName)
{
   ...
   if (ZString(val.asLong())==val) return Integer;
   if (val.isPrintable()) return String;
   return Binary;
#endif
} // valueType
...
ZExport(ZString) ZProfile::value(const ZString& aValueName)
{
...
#endif
#ifdef ZC_OS2
   if (!iPath.size()) ZTHROWEXC(MissingPath);
   if (!aValueName.size()) ZTHROWEXC(MissingValueName);
   openRoot();
   ...
   if (size > 0) {
      ...
      ERRORID err = WinGetLastError(iHab);
      if (err == 0) {
         if (buf[buf.size()]==0 && buf.subString(1, buf.size()-1).isPrintable())
...
ZExport(long) ZProfile::longValue(const ZString& aValueName)
{
   ...
   if (dataType != 4) ZTHROWEXC(UnsupportedDataType);
   return data;
#endif
#ifdef ZC_OS2
...
ZExport(ZProfile&) ZProfile::setValue(const ZString& aValue, const ZString& aValueName, int aType)
{
   ZFUNCTRACE_DEVELOP("ZProfile::setValue(const ZString& aValue, const ZString& aValueName, int aType)");
   ...
   if (valueExists(aValueName)) deleteValue(aValueName);

   if (!RegSetValueEx(
          iPathHandle, aValueName, 0,
          dataType, (LPBYTE)(const char*)aValue, aValue.size()) == ERROR_SUCCESS)
...
ZExport(ZProfile&) ZProfile::setValue(long aValue, const ZString& aValueName)
{
   ZFUNCTRACE_DEVELOP("ZProfile::setValue(const ZString& aValue, const ZString& aValueName, int aType)");
#ifdef ZC_WIN
   openPath(zTrue);
...
#ifdef ZC_WIN
   openPath(zFalse);
   RegDeleteValue(iPathHandle, aValueName);
#endif
#ifdef ZC_OS2
   if (!iPath.size()) ZTHROWEXC(MissingPath);
   if (!aValueName.size()) ZTHROWEXC(MissingValueName);
...
ZExport(ZProfile&) ZProfile::deleteKey(const ZString& aKeyName)
{
   ...
   setPath(oldPath);
   openPath(zFalse);
   RegDeleteKey(iPathHandle, aKeyName);
#endif
#ifdef ZC_OS2
   if (iPath.size() && aKeyName.size()) return *this; // no keys at this level
   ...
   if (!iPath.size() && !aKeyName.size()) {
      ...
      if (iRootHandle == HINI_SYSTEMPROFILE ||
          iRootHandle == HINI_USERPROFILE) return *this;
      ...
      if (!PrfWriteProfileString(
           iRootHandle,
           iPath.size() ? iPath : aKeyName,
           0, 0))
         throwSysErr(PrfWriteProfileStringName);
   } // if
#endif
